  ...
  const [signup, { loading }] = useMutation(SIGNUP_MUTATION, {
    onCompleted: ({ createUser }) => {
      ...
      if (createUser.success) {
        setSignupSuccess(createUser.message)
        setSignupError(null)
        addNotification(createUser.message)
      } else {
        setSignupError(createUser.message)
        setSignupSuccess('')
      }
    },
    onError: (error) => {
      const message = error.message || 'Signup failed. Try again.'
      setSignupError(message)
      setSignupSuccess('')
      addNotification(`Something went wrong: ${message}`, 3000, 'error')
    },
  })
